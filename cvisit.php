	...
	if (! (isset($_POST["firstname"]) && isset($_POST["lastname"]) && isset($_POST["date"]) && isset($_POST["reasonforvisit"]) && isset($_POST["related"]))) {
		...
		if ($ok == 1) {
			...
			if ($date_current > $date_format) {
				$message = "Atentie! Programarea trebuie facuta cu cel putin o zi inainte!";
			} 

			else {
				$date_string = date_format($date_format,"Y/m/d H:i:s");

				$query = $conn->prepare('SELECT count(id) as "nr" FROM programari where id_vizitator like ? and id_detinut like ? and data_vizitei like ?');
				...
				$row = $result->fetch_assoc();
 				if ($row['nr'] > 0) {
					$message = "Atentie! Aveti deja o programare pentru data respectiva la detinutul $lastname $firstname";
				}
					...
					$result = $query->get_result();
					$message = "Programare introdusa cu succes pentru data de ".$date;
				}
			}	
		}
	}	
